...
func InitTransaction() {
	contract, _, closeFunc := gateway.CreateNewConnection()
	defer closeFunc()

	result, err := contract.SubmitTransaction("InitTransaction", "did:type:1", "100")
	if err != nil {
		panic(err)
	}
	fmt.Println(result)
...
func DeleteAllTransactions() {
	...
	_, err := contract.SubmitTransaction("DeleteAllTransactions")
	if err != nil {
		panic(err)
	}
	fmt.Println("DeleteAllTransactions success")
}
